...
async fn pg_store_full_integration() {
    ...
    sqlx::query("DELETE FROM secrets").execute(&raw).await.unwrap();
    sqlx::query("DELETE FROM secret_meta").execute(&raw).await.unwrap();
    ...
    assert_eq!(store.put_secret("db/prod/pw", &c1, "alice", now).await.unwrap(), 1);
    assert_eq!(store.put_secret("db/prod/pw", &c2, "bob", now + 10).await.unwrap(), 2);

    let latest = store.get_latest("db/prod/pw").await.unwrap().unwrap();
    assert_eq!(latest.version, 2);
    assert_eq!(latest.created_by, "bob");
    assert_eq!(cipher.open_secret(&latest.ciphertext).unwrap(), "db-password-2");
    ...
    store.put_secret("api/key", &c3, "alice", now + 20).await.unwrap();
    ...
    assert!(!stored.contains("db-password-2"), "plaintext must never be stored");
    assert_eq!(cipher.open_secret(&stored).unwrap(), "db-password-2");
