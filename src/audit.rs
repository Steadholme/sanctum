...
//!
//! Mirrors the estate's canonical sink (keystone): [`AuditSink`] holds a bounded `tokio::mpsc`
//! sender drained by a spawned worker that POSTs each event to `WATCHTOWER_URL/events`
...
//! worker.
//!
//! A SECRET VALUE never rides an event: producers fill only the logical fields (actor / action /
//! target=path / severity / detail), and `detail` is kept short and value-free at each call site.
//! The reveal audit records WHO revealed WHICH path WHEN — never the plaintext.
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::notice("secret.reveal", "a@b", "db/pw", "ok"));
        }
    ...
    /// A reveal event serializes to exactly the safe shared fields — and NEVER the secret value.
    #[test]
    fn reveal_serializes_without_value() {
        let ev = AuditEvent::notice("secret.reveal", "alice@w33d.xyz", "db/prod/password", "v3");
        ...
        assert_eq!(v["action"], "secret.reveal");
        assert_eq!(v["target"], "db/prod/password");
        assert_eq!(v["severity"], "notice");
        assert_eq!(v["source"], "sanctum");
        // The secret plaintext is absent by construction.
        for secret in ["hunter2", "s3cr3t-value", "correct horse battery staple"] {
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::notice("secret.reveal", "u", "db/pw", "v1"));
        }
        assert!(
            sink.dropped() > 0,
